...
def export_to_solc(crytic_compile: "CryticCompile", **kwargs: str) -> List[str]:
    ...
    if len(crytic_compile.compilation_units) == 1:
        ...
        if path := export_to_solc_from_compilation_unit(
            compilation_unit, "combined_solc", export_dir
        ):
            return [path]
        return []
    ...
    for key, compilation_unit in crytic_compile.compilation_units.items():
        if path := export_to_solc_from_compilation_unit(
            compilation_unit, key, export_dir
        ):
            paths.append(path)
    return paths
...
class Solc(AbstractPlatform):
    ...
    def compile(self, crytic_compile: "CryticCompile", **kwargs: str) -> None:
        ...
        solc_working_dir = kwargs.get("solc_working_dir")
    ...
    if "contracts" in targets_json:
        ...
        for original_contract_name, info in targets_json["contracts"].items():
            ...
            compilation_unit.abis[contract_name] = (
                info["abi"] if is_above_0_8 else json.loads(info["abi"])
            )
            ...
            userdoc = (
                info["userdoc"]
                if is_above_0_8
                else json.loads(info.get("userdoc", "{}"))
            )

            devdoc = (
                info["devdoc"]
                if is_above_0_8
                else json.loads(info.get("devdoc", "{}"))
            )
...
def is_optimized(solc_arguments: Optional[str]) -> bool:
    ...
    :return:
    """
    return "--optimize" in solc_arguments if solc_arguments else False
...
def _run_solc(
    ...
    old_04_versions = [f"0.4.{x}" for x in range(12)]
    ...
    if solc_arguments:
        ...
        solc_args_ = [f"--{x}".split(" ", 1) for x in solc_args if x]
    ...
    if (
        compiler_version.version not in [f"0.4.{x}" for x in range(11)]
        and "--allow-paths" not in cmd
    ):
...
def _run_solcs_env(
    ...
    guessed_solcs = _guess_solc(filename, working_dir)
    for guessed_solc in guessed_solcs:
        if solcs_env and guessed_solc not in solcs_env:
            continue
        try:
    ...
    if not targets_json:
        solc_versions_env = solcs_env or []
